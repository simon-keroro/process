...
V4.0 覆盖“合同流程”和“设备采购流程”，分别依据 `0-合同流程-V01.docx`、`0-设备采购流程-V01.docx` 整理，并按用户确认更新金额规则。使用右上角按钮切换业务流程，再选择金额情形；点击节点查看责任角色、系统、材料和提醒。

V4.0 合同流程将原“10 万元以内”调整为“20 万元以内”，原“10–50 万元”调整为“20 万以上 50 万以内”。20 万元归入前一档；各档办理步骤保持不变。
...
合同数据与公共展示逻辑位于 `dist/app.js`，设备采购数据位于 `dist/equipment.js`。两种流程共用页面、节点详情和样式。发布时同步更新 `index.html` 中全部资源版本参数（当前为 `v=4.0`），并在 VPS 重新构建容器。
# process
